...
class GPORTAL_AUTH
{
            ...
            if ($httpCode === 302) {
                ...
                if ($tokenHttpCode !== 200) {
                    $JSON = (object)[
                        'type' => 'error',
                        'title' => 'ERROR',
                        'message' => "Failed To Login, Access Token, Status Code"
                    ];
                    return json_encode($JSON);
                }
                ...
                $_SESSION[$this->sessionEmailKey] = $username;
                ...
                $JSON = (object)[
                    'type' => 'error',
                    'title' => 'ERROR',
                    'message' => "Failed To Login, Status Code"
                ];
                return json_encode($JSON);
            } else {
                $this->log("Authentication Failed, HTTP Status Code: $httpCode");
    ...
    public function fetchServers()
    {
        ...
        foreach ($urls as $region => $url) {
            $this->log("Fetching Servers From $region");
            ...
            if (curl_errno($ch)) {
                $error = curl_error($ch);
                $this->log("Failed To Fetch Servers From $region: $error");
                curl_close($ch);
                throw new Exception("Failed To Fetch Servers From $region: $error");
            }

            $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            if ($httpCode !== 200) {
                $this->log("Failed To Fetch Servers From $region, HTTP Status Code: $httpCode");
                curl_close($ch);
                throw new Exception("Failed To Fetch Servers From $region, HTTP Status Code: $httpCode");
            }
            ...
            if (!is_array($regionServers)) {
                $this->log("Invalid Response Format From $region Servers!");
                throw new Exception("Invalid Response Format From $region Servers!");
            }
